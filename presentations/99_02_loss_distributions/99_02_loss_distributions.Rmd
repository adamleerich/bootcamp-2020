...
Let's start with some real loss data.

```{sql eval=FALSE}
select ...
  from Report.ClaimExposureSectionCombinedView
 where TriFocusName in ('EPL', 'BICI EPL MM', 'BUSA EPL MM')
   and ExposureYOA between 2010 and 2019
   and left(ExposureStatus, 1) = 'C'
   and BlockIndicator = 0
   and coalesce(LimitInUSD, 0) > 0
   and coalesce(ExcessInUSD, DeductibleInUSD, 0) > 0
   and SlipOrderTotalPaid > 0
   and SettlementCurrency = 'USD';
```
...
epl_cols <- cols(
  TriFocusName = col_character(),
  ExposureYOA = col_character(),
...
epl <- readr::read_csv(epl_path, col_types = epl_cols)
```

```{r eval=FALSE}
library(tidyverse)
epl <- readr::read_csv('epl-for-r-training.csv')
```

```{r}
dim(epl)
table(epl$ExposureYOA)
```
...
info(epl) %>% 
  filter(type != 'numeric') %>% 
  dplyr::select(column, type, na_count, unique_values) %>% 
  knitr::kable(digits = 0,format.args = list(big.mark = ','))
```
...
::: {.column}
```{r echo=FALSE}
hist(epl$SlipOrderTotalPaid, breaks = 20)
```
:::
::: {.column}
```{r echo=FALSE}
hist(log(epl$SlipOrderTotalPaid), breaks = 20)
```
:::
::::::::::::::
...
# Indemnity + Defence
```{r echo=FALSE}
x <- epl$SlipOrderIndemnityPaid + epl$SlipOrderDefencePaid
y <- x[x > 0]
```
:::::::::::::: {.columns}
::: {.column}
...
```{r}
exp(5.8)
x <- round(epl$SlipOrderFeesPaid / 100, 0) * 100
table(x[x < 1001])
```
...
let's look at indemnity + defence only.

```{r}
epl$IndemnityAndDefence <- 
  epl$SlipOrderIndemnityPaid + epl$SlipOrderDefencePaid
x <- epl$IndemnityAndDefence[epl$IndemnityAndDefence > 0]
mean(x)
sd(x)
```
